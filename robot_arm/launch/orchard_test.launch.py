"""
Sanity Test #1: M1013 in orchard.world

Same launch logic as bot.launch.py but uses orchard.world.
Robot spawns near a tree row for visual sanity check.
Husky is NOT included yet — that's F1.4.

Usage:
    ros2 launch robot_arm orchard_test.launch.py
"""
...
def generate_launch_description():
    ...
    # Same URDF cleanup as bot.launch.py
    ...
    # Resource paths so Gazebo can find package://robot_arm/meshes/...
    ...
    # Spawn M1013 at world origin (0,0,0).
    # Orchard tree bbox is X[4,41] Y[3,35], so robot will be at the SW corner
    # of the orchard with all trees visible in front.
    # (TF stays consistent: URDF "world" link == Gazebo world frame)
    spawn_entity = Node(
        package='ros_gz_sim',
        executable='create',
        arguments=[
